...
# --- 다음 질문 가져오기 (중복 제외) ---
def get_next_question(db: Session, answered_ids: Optional[List[int]] = None) -> Optional[MbtiQuestion]:
...
def submit_answer(db: Session, user_id: int, question_id: int, choice: str):
    ...
    # dimension → 컬럼 매핑
    dim_map = {"E/I": ("E", "I"), "S/N": ("S", "N"), "T/F": ("T", "F"), "J/P": ("J", "P")}
    if question.dimension not in dim_map:
        raise HTTPException(status_code=400, detail="잘못된 dimension")

    col = dim_map[question.dimension][0] if choice.upper() == "A" else dim_map[question.dimension][1]
...
def get_mbti_result(db: Session, user_id: int):
    ...
    recommendations = {
        "ISTJ": {"stocks": ["삼성전자", "SK하이닉스"], "etf": ["TIGER 200"], "desc": "철저한 분석형. 안정적 투자."},
        "ISFJ": {"stocks": ["LG생활건강", "CJ제일제당"], "etf": ["KODEX 배당"], "desc": "조용하지만 꾸준한 수익형."},
        "INFJ": {"stocks": ["네이버", "카카오"], "etf": ["TIGER 콘텐츠"], "desc": "트렌드를 깊게 이해하는 스타일."},
        ...
        "ENFP": {"stocks": ["테슬라", "루시드"], "etf": ["ARKK"], "desc": "혁신 미래 열정형."},
        "ENTP": {"stocks": ["엔비디아", "슈퍼마이크로"], "etf": ["SOXL"], "desc": "기회 포착형."},
        "ESTJ": {"stocks": ["삼성전자", "현대차"], "etf": ["KODEX 200"], "desc": "대장주 안정형."},
        "ESFJ": {"stocks": ["CJ ENM", "롯데칠성"], "etf": ["소비재 ETF"], "desc": "분위기·커뮤니티 반영."},
        "ENFJ": {"stocks": ["애플", "MSFT"], "etf": ["QQQ"], "desc": "장기적 비전 투자."},
        "ENTJ": {"stocks": ["아마존", "메타"], "etf": ["SPY"], "desc": "전략적 빅테크 투자."},
    }
