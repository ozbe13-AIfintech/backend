...
def search(db: Session, query: str, skip: int = 0, limit: int = 50):
    ...
    stock_query = db.query(
        ...
        literal_column("NULL").label("base_currency"),  # 다른 테이블에 없는 컬럼은 NULL로 채움
    ...
    index_query = db.query(
        ...
        Index.name.ilike(f"%{query}%") |
        Index.symbol.ilike(f"%{query}%")
    )
    ...
    results = stock_query.union_all(index_query).union_all(exchange_rate_query).union_all(exchange_rate_history_query)
