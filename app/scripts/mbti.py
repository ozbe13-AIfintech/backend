...
QUESTIONS = [
    ...
    (
        "E/I",
        "주식 커뮤니티에서 너의 태도는 어떤 편이냐?",
        "주식방에서 떠드는 게 반쯤 취미다",
        "주식방 알림 뜨면 심장도 같이 뜨는 타입이다",
    ),
    (
        "E/I",
        "지인이 종목을 추천했을 때 너는 어떻게 반응하냐?",
        "‘오? 왜?’ 하며 관심 생김",
        "‘근거는?’ 모드 들어감",
    ),
    (
        "E/I",
        "사람들과 함께 시장 분석하는 것에 대해 어떻게 생각하냐?",
        "사람들과 같이 분석하는 게 좋다",
        "혼자 집중해서 차트 볼 때 행복하다",
    ),
    (
...
def seed_questions():
    ...
    for dimension, text, option_a, option_b in QUESTIONS:
        exists = (
            db.query(MbtiQuestion)
            .filter(MbtiQuestion.dimension == dimension, MbtiQuestion.text == text)
            .first()
        )
        if exists:
